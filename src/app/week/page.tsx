...
export default async function WeekPage() {
  ...
  // Mason's account only — untouched, still driven by the global day_types
  // table exactly as before. See isLegacyAccount.
  if (legacy) {
  ...
  // Every other account — driven by that account's own profile baseline and
  // custom_day_types/custom_day_type_selections, never the global day_types
  // table.
  const [{ data: dailyLogs }, { profile }, { customDayTypes }, { selectionsByDate }] =
    await Promise.all([
  ...
  const customDayTypesById = new Map((customDayTypes ?? []).map((d) => [d.id, d]));

  const days = weekDates.map((date, i) => {
    const log = logsByDate.get(date);
    const actual = log ? (totalsByDailyLogId.get(log.id) ?? emptyMacros()) : emptyMacros();
    const selectedTypes = (selectionsByDate?.[date] ?? [])
      .map((id) => customDayTypesById.get(id))
      .filter((d): d is CustomDayType => d !== undefined);
    ...
    return {
      ...
      label: DAY_LABELS[i],
      isFuture: date > today,
      typeLabel: selectedTypes.length ? selectedTypes.map((d) => d.name).join(", ") : "Baseline",
      goingOut: log?.going_out_flag ?? false,
      actual,
      target,
    };
  ...
  const countedDays = days.filter((d) => d.target && !d.isFuture);
  const goingOutCount = days.filter((d) => d.goingOut && !d.isFuture).length;
  ...
  const avgTarget: Macros = {
    calories: avg((d) => d.target!.calories),
    protein: avg((d) => d.target!.protein),
    carbs: avg((d) => d.target!.carbs),
    fat: avg((d) => d.target!.fat),
  };
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <div className="space-y-6">
              ...
              <tbody>
                {days.map((d) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2">{d.target ? d.typeLabel : "—"}</td>
                    <td className="px-3 py-2">
                      {Math.round(d.actual.calories)}
                      {d.target && (
                        <span className="text-neutral-400"> / {Math.round(d.target.calories)}</span>
                      )}
                    </td>
                    <td className="px-3 py-2 text-neutral-500">
                      {Math.round(d.actual.protein)}g / {Math.round(d.actual.carbs)}g /{" "}
          ...
          >
            {countedDays.length === 0 ? (
              <p className="text-sm text-neutral-400">
                Set up your goals to see weekly averages here.
              </p>
            ) : (
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Calories</span>
                  <span>
                    {Math.round(avgActual.calories)} avg vs {Math.round(avgTarget.calories)} target
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Protein</span>
                ...
                <div className="flex justify-between">
                  <span>Carbs</span>
                  <span>
                    {Math.round(avgActual.carbs)}g avg vs {Math.round(avgTarget.carbs)}g target
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Fat</span>
